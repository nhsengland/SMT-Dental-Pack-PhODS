...
monthly_delivered_pre <- read.csv("N:/_Everyone/Primary Care Group/SMT_Dental Calendar data format/BSA Calendar data/NPP/npp_pre_intro.csv") %>% 
  ...
  if(level == "National"){
    ...
    data_post <- monthly_delivered_post %>% 
      ...
      data_pre <- monthly_delivered_pre %>% 
        ...
        group_by(REGION, TREATMENT_MONTH) %>% 
      ...
      data_post <- monthly_delivered_post %>% 
        ...
        data_pre <- monthly_delivered_pre %>% 
          filter(REGION == region_STP_name) %>% 
          group_by(COMMISSIONER_NAME, TREATMENT_MONTH) %>% 
          summarise(total = sum(total)) %>% 
          mutate(financial_year = ifelse(TREATMENT_MONTH <= "202402",
                                         "2023/24", "2024/25"), 
                 group = ifelse(TREATMENT_MONTH <= "202402",
