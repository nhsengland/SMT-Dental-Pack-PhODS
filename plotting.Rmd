...
latest_month <- max(UDA_calendar_data$month)
...
fy <- get_financial_year(latest_month)
...
plot_historic_commissioned_UDA_wd <- function(data = UDA_calendar_data,
  ...
  data_try <- data %>%
    ...
    summarise(#UDA_delivery = sum(UDA_delivered, na.rm = TRUE),
              contracted_UDAs = sum(annual_contracted_UDA, na.rm = TRUE)) %>%filter(month >= "2023-04-01") %>%
    left_join(working_days,by=c('month')) %>% 
    mutate(std_UDA_delivered = (contracted_UDAs*(`no workdays`/`total workdays`))) %>%
    filter(!is.na(month))%>%
    select(month,contracted_UDAs)
...
p2 <- ggplot() +
  geom_line(data = data_try, linetype = 1, 
            aes(x = month, y = contracted_UDAs), colour = "black") +
  ylab("commissioned UDAs") +
  xlab(NULL) +
  #geom_text(data = data_try,
  #          aes(x = month, y = contracted_UDAs + min(contracted_UDAs)/450, label = round(contracted_UDAs, 0)),
  #          size = 2.5,check_overlap = TRUE) +
  scale_x_date(breaks = "2 month", date_labels = "%b %Y") +
  #scale_y_continuous(limits = c(min(c(data_try$contracted_UDAs, 95), na.rm = T) - 5,                                                           #                        max(c(data_try$contracted_UDAs, 95), na.rm = T) + 5)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = -0.0001),
        legend.position = "none")

# Define title and subtitle
main_title <- "Monthly total commissioned UDAs"
...
plot_unique_patients_rolling <- function(data = unique_patients_rolling, 
  ...
  data <- pull_unique_patients()
   ...
   data$region_name <- str_to_title(data$region_name)
...
if(level == "Regional"){
    subtitle <- region_STP_name
     ...
     # select regional unique patient numbers
    new_data <- data %>% 
      filter(region_name == region_STP_name) %>% 
