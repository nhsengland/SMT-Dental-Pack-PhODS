...
get_financial_year <- function(month = month){
  ifelse(month(test1)>3, 
       paste(year(test1), "/", substr(year(test1)+1,start=3,stop=4), sep = ""), 
       paste(year(test1)-1, "/", substr(year(test1),start=3,stop=4), sep = ""))
}
```
...
Table_UDA_UOA_delivery_all_ICBs <- function(data = UOA_calendar_data,
  ...
  if(UDAorUOA == "UDA"){
   ...
   data <- data %>%
    ...
    data <- data %>% 
        mutate(financial_year = case_when(
               calendar_month >= as.Date("2020-04-01") & calendar_month < as.Date("2021-04-01") ~ "2020/21",
               calendar_month >= as.Date("2021-04-01") & calendar_month < as.Date("2022-04-01") ~ "2021/22",
               calendar_month >= as.Date("2022-04-01") & calendar_month < as.Date("2023-04-01") ~ "2022/23",
               calendar_month >= as.Date("2023-04-01") & calendar_month < as.Date("2024-04-01") ~ "2023/24",
               calendar_month >= as.Date("2024-04-01") & calendar_month < as.Date("2025-04-01") ~ "2024/25"))
...
plot_historic_UOA_delivery_wd <- function(data = UOA_calendar_data,
  ...
  data_try <- data %>%
    ...
    # only include current FY data -- calculate percentage of UDA delivered -- standardized by no of working days
    filter(month >= as.Date (current_fy_start)) %>% ## this needs to be updated every FY
    left_join(working_days,by=c('month')) %>% 
  ...
  # Define plot 2
  p2 <- ggplot() +
    geom_line(data = data2, aes(x = month, y = perc_UOA_delivered), colour = "black") +
    geom_line(data = data2, aes(x = month, y = line_for_100), colour = "blue") +
