...
plot_YTD_UDA_UOA_delivery <- function(data = UDA_calendar_data, 
  ...
  if(plotChart == TRUE){
    #plot code
    p <- ggplot(data, aes(x = month, y = value/1000000, colour = measure)) +
        theme_bw() +
        geom_line(aes(linetype = measure),
                  size = 1) +
        geom_point() +
      geom_text_repel(aes(label = prettyNum(value, big.mark=",")), data = data_labels, colour = "black", position = "dodge") + 
...
plot_banded_CoT <- function(data = UDA_calendar_data_FD, 
  ...
  if(plotChart == TRUE){
    ...
    #plot code
    ggplot(data, aes(x = month, y = CoTs, colour = band)) +
      theme_bw() +
      theme(axis.text.x = element_text(angle = 90)) +
      geom_line(size = 1) +
      geom_point(size = 1) +
      geom_text_repel(aes(label = prettyNum(CoTs, big.mark=",")), data = data_labels, colour = "black", position = "dodge") +
      scale_x_date(date_breaks = "1 month",
                   date_labels = "%b-%y") +
...
if(plotChart == TRUE & asIndex == FALSE){
  ...
  #plot code
  ggplot(data, aes(x = month, y = total_unique_patients, colour = name)) +
    theme_bw() +
    geom_line() +
    geom_point() +
    geom_text_repel(aes(label = prettyNum(total_unique_patients, big.mark=",")), data = data_labels, colour = "black", position = "dodge") + 
...
#plot code
ggplot(data, aes(x = month, y = value, colour = variable)) +
  theme_bw() +
  geom_line() +
  geom_point() +
    geom_text_repel(aes(label = paste0(round(value), "%", sep = "")), 
                    data = data_labels, colour = "black", position = "dodge") + 
  scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE)) +
  scale_colour_manual(values = get_colour_palette()) +
  labs(title = title,
       x = "12 month rolling period end date",
...
plot_DCP_analysis <- function(data = UDA_calendar_data,
  ...
  delivery_total <-  UDA_calendar_data %>% 
    group_by(month) %>%
