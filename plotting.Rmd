...
plot_UDA_UOA_delivery_wd <- function(data = UDA_calendar_data, 
  ...
  #include data after 2021-04-01 (can be changed)
  data <- data %>%
...
#method 2 -- to calculate working days
  ...
  if(plotChart == TRUE){
    ...
    #what if plotChart == TRUE -- want to create a table instead of a plot
    if(UDAorUOA == "UDA"){
...
plot_UDA_delivery_all_regions <- function(data = UDA_calendar_data, 
                                              UDAorUOA = "UDA",
                                              level = "National",
                                              region_STP_name = NULL,
                                              plotChart = TRUE, 
...
plot_historic_UDA_delivery_wd <- function(data = UDA_calendar_data,
  ...
  historic_data <- data %>%
    select(month, contract_number, commissioner_name, region_name,
           annual_contracted_UDA, UDA_delivered)
  #create a new field as financial year and then calculate full year UDA delivered, contracted UDA and mutate a new field about percentage of UDA delivered
  ...
  data_try <- data %>%
    ...
    mutate(perc_UDA_deliverd = (UDA_delivery /(contracted_UDAs*(`no workdays`/`total workdays`))*100)) %>%
...
plot_historic_UOA_delivery_wd <- function(data = UOA_calendar_data,
  ...
  data_try <- data %>%
   ...
   # mutate(scaled_UOA_delivery = UOA_delivery * 12) %>%
    mutate(perc_UOA_delivered = (UOA_delivery / (contracted_UOAs*(`no workdays`/`total workdays`))*100)) %>%
