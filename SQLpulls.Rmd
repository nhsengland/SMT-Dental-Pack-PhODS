...
UDA_calendar_data_FD_only<-pull_UDA_calendar_data_FD_only()%>%
  inner_join(contractual_dataset,by=c('YEAR_MONTH','CONTRACT_NUMBER'))
#exclude those with UDA target less than 1 - rename fields
UDA_calendar_data_FD_only = dplyr::filter(UDA_calendar_data_FD_only, as.numeric(UDA_PERF_TARGET) > 1.0)
...
pull_unique_patients <- function(){
  ...
  result <- dbSendQuery(con, sql)
  unique_patients <- dbFetch(result)
  dbClearResult(result)
...
pull_dcp_data <- function(){
  ...
  sql <- "SELECT *
  FROM [NHSE_Sandbox_PrimaryCareNHSContracts].[Dental].[Calendar_DCP]
  "
  result <- dbSendQuery(con, sql)
