...
-   This graph shows the percentage of CoTs carried out which benefitted from involvement by Dental Care Practitioners.
...
-   This chart shows the percentage of UDAs carried out which benefitted from involvement by Dental Care Practitioners by treatment band.
-   DCP assistance data has been collected since October 2022 and DCP lead data collected since April 2024 so will likely take time for providers to improve reporting on it.
...
```{r time series comparison UOA, echo = FALSE, warning = FALSE, message = FALSE, fig.width=14}
plot <- plot_time_series_comparison(UDAorUOA = "UDA")
table <- table_time_series_comparison(UDAorUOA = "UDA")

grid.arrange(plot, table, nrow = 1, widths = c(1, 1, 1), 
             layout_matrix = rbind(c(1, 1, 2)))
```

## Comparison of monthly UOA data

Spikes are also smoothed out in UOA data. UOA calendar data is incomplete for the latest financial year and we expect final data to be higher.

```{r time series comparison, echo = FALSE, warning = FALSE, message = FALSE, fig.width=14}
plot <- plot_time_series_comparison(UDAorUOA = "UOA")
table <- table_time_series_comparison(UDAorUOA = "UOA")
